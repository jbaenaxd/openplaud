            ...
            if (result.ok && result.result.length > 0) {
                for (const update of result.result as TelegramUpdate[]) {
                    ...
                    if (
                        ...
                        !allowedUsers.includes(message.from.id.toString())
                    ) {
                        console.log(
                            `🚫 Unauthorized Telegram user: ${message.from.id}`,
                        );
                        continue;
                    }
                    ...
                    if (audio) {
                        console.log(
                            `📥 Received audio from Telegram user ${message.from.id}`,
                        );
                        ...
                        try {
                            ...
                            const storageProvider =
                                await createUserStorageProvider(targetUser.id);

                            const recordingId = nanoid();
                            const timestamp = new Date(message.date * 1000);
                            const filename =
                                (audio as { file_name?: string }).file_name ||
                                `telegram-${recordingId}.ogg`;
                            ...
                            console.log(
                                `✅ Telegram recording saved: ${recordingId}`,
                            );
